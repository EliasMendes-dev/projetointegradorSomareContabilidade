...
namespace projetointegrador
{
    public partial class FrmTelaPagamentos : Form
    {
        ...
        private void FrmTelaPagamentos_Load(object sender, EventArgs e)
        {
            ...
            comboAno.Items.Insert(0, "Todos os anos");
        ...
        private void Cadastro_ContaSalvaComSucesso(object sender, EventArgs e)
        {
            ...
            // Atualize os combos antes de atualizar os gráficos
            PreencherFiltros();
            PreencherClientes();

            if (comboAno.Items.Count > 0 && comboAno.SelectedIndex == -1)
                comboAno.SelectedIndex = 0;
            if (comboCliente.Items.Count > 0 && comboCliente.SelectedIndex == -1)
                comboCliente.SelectedIndex = 0;

            AtualizarGrafico();                    // Pie de categorias
            AtualizarGraficoReceitaDespesaPie();   // Pie de Receita×Despesa
            Relatorios();
        ...
        {
            // 1) Tenta ler o ano; se não tiver ou for inválido, aborta sem desenhar
            if (comboAno.SelectedItem == null
                || !int.TryParse(comboAno.SelectedItem.ToString(), out int ano))
            {
                return;
            }

            // 2) Como antes, lê os demais filtros
            string tipo = comboTipo.Text;
            bool isAnual = comboVisualizacao.Text == "Anual";
            ...
            // 4) Garante dicionário válido
            if (dados == null)
                dados = new Dictionary<string, decimal>();
        ...
        {
            // 1) Tenta ler o ano como antes; se falhar, limpa e retorna
            if (comboAno.SelectedItem == null
                || !int.TryParse(comboAno.SelectedItem.ToString(), out int ano))
            {
                return;
            ...
            // 4) Soma totais
            decimal totalRec = recPeriodo?.Values.Sum() ?? 0m;
            decimal totalDesp = despPeriodo?.Values.Sum() ?? 0m;
        ...
        private void PreencherClientes()
        {
            ...
            comboCliente.Items.Clear(); // Limpa todos os itens antes de adicionar novamente
